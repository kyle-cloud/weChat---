...
public class JDBCInfo {
	...
	private JDBCInfo(){
		...
		try {
			InputStream inStream = JDBCInfo.class.getClassLoader().getResourceAsStream("jdbc.properties");
			prop.load(inStream);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public String addConfig2jdbcUrl(){
		return Toolkit.cutStringEnd(this.getJdbcUrl(), "?") + MYSQL_CONFIG;
	}
	...
	public void saveJDBCConfig() throws Exception{
		...
		if ( InstallDatabase.isH2Database() ) {
			prop.setProperty("useC3PO", "true");
			prop.setProperty("jdbcUrl", this.getJdbcUrl());
			prop.store(oFile,"");
		} else {
			prop.setProperty("useC3PO", "true");
	...
	public void saveC3P0Config() throws Exception{
		...
		if ( InstallDatabase.isH2Database() ) {
			prop.setProperty("c3p0.jdbcUrl", this.getJdbcUrl());
			prop.store(oFile,"");
		} else {
